...
<tr>
      ...
      <td>
        이론적으로만 알고 있던 개념들을 직접 실습해 보니 한층 더 이해하기 쉬웠다. 특히 데이터베이스 분야에서는 SQL 쿼리문 작성 방법만 알고 있었는데, 이번 프로젝트를 통해 흐름과 과정을 배울 수 있었다.
첫 프로젝트이다 보니 어떻게 진행해야 할지 고민이 많았지만, 팀원들의 도움을 받으며 함께 협업한 덕분에 프로젝트를 잘 마무리할 수 있었다고 생각한다. 또한 이번 프로젝트를 통해 팀원들과 소통하는 방법과 협업 도구를 활용하는 방법을 자세히 배울 수 있었기에, 앞으로 남은 프로젝트들에서도 이러한 경험을 적극 활용하고 싶다는 생각이 들었다.
      </td>
    </tr>
...
<tr>
      ...
      <td>
        요구사항 명세서는 우리가 정한 시나리오 기반으로 만들면 되니까 생각보다 간단할거라고 생각했는데 사용자와 관리자 관점으로 나눠서 생각하니까 다른부분이 있어서 더 고민했고, 이후에  ERD와 테이블명세서가 나온 후 제대로 반영이 됐는지 또는 추가된 사항이 있는지 확인하는 과정에서 내가 의도한것과 다른부분이나 더 추가된 부분에 대해 부족한 점이 있었나 생각해보게 되었다. 테이블 명세서 기반으로 DB에 테이블을 생성하고 데이터를 넣는 과정에도 오류가 많이나서 고쳐나가면서 더 공부할 수 있던 프로젝트였다.
      </td>
    </tr>
